...
export function resolveByAnswer(
  ...
  if (["bearing", "sealing", "rotating"].includes(functionType)) {
    return {
      status: "keep",
      caution: "핵심 기능면입니다. 완화 전 CAE, 조립성, 시험 근거 확인이 필요합니다.",
    };
  }
  ...
  if (["fastening", "cover", "cosmetic"].includes(functionType)) {
    return {
      ...
      caution: "체결/커버 기능이면 완화 여지가 있습니다. 간섭과 이탈 가능성을 먼저 확인하세요.",
    };
  }

  return {
    status: "need_check",
    caution: "기능 정의가 필요합니다. 위치 결정, 압입, 반복 접촉, 회전체 여부를 확인하세요.",
  };
}
...
function scoreRisk(type: ToleranceType, value: number): { riskLevel: RiskLevel; riskScore: number; reason: string } {
  if (type === "fit") {
    if (value <= 6) {
      return {
        riskLevel: "critical",
        riskScore: 90,
        reason: "H6 이하 끼워맞춤은 정밀 보어 가공과 반복 측정이 필요합니다. 기능 근거 없이 지정하면 비용이 크게 증가합니다.",
      };
    }
    if (value <= 7) {
      return {
        riskLevel: "high",
        riskScore: 74,
        reason: "H7은 베어링 압입 또는 위치 결정 기능에서 자주 쓰입니다. 단순 체결 구멍이라면 완화 후보입니다.",
      };
    }
    return {
      riskLevel: "medium",
  ...
  if (type === "roughness") {
    if (value <= 0.4) return high("Ra 0.4 이하는 연삭, 래핑, 추가 검사 가능성이 큽니다.", 86);
    if (value <= 0.8) return high("Ra 0.8은 접촉면, 반복면, 베어링면인지 확인해야 합니다.", 68);
  ...
  if (["position", "profile", "runout", "total_runout", "concentricity", "cylindricity"].includes(type)) {
    ...
    if (value <= 0.05) return high("GD&T 0.05 이하에서는 제조와 검사 난이도가 높습니다. 기능 근거 확인이 필요합니다.", 74);
  ...
  if (value <= 0.02) return high("+/-0.02mm는 일반 CNC보다 정밀 관리가 필요한 수준입니다.", 74);
  if (value <= 0.05) return medium("+/-0.05mm는 기능에 따라 일반 공차로 완화 가능성을 검토할 수 있습니다.", 45);
  return low("일반 가공 공차 범위로 판단합니다.", 18);
}
...
function recommend(
  ...
  if (type === "fit") {
    return {
      ...
      caution: "끼워맞춤은 기능 확인 없이 완화하면 안 됩니다.",
    };
  }
  ...
  if (type === "roughness") {
    return {
      ...
      caution: "반복면, 슬라이딩면, 베어링 접촉면이면 유지해야 합니다.",
    };
  }
  ...
  if (value <= 0.02) {
    return {
      ...
      caution: "기능 요구와 조립/CAE 검증이 필요합니다.",
    };
  }
  ...
  if (value <= 0.05) {
    return {
      ...
      range: "0.08-0.15 후보",
      caution: "체결 구멍 또는 커버면이면 완화 가능성이 있습니다.",
    };
  }

  return {
    status: "relax_candidate",
    range: "현재 수준 유지 또는 일반 공차 검토",
...
function buildQuestions(input: RuleInput): string[] {
  const questions = [
    "이 공차가 베어링, 반복 접촉, 회전 정렬, 위치 결정 중 어떤 기능과 직접 연결됩니까?",
    "현재 공차를 지정한 근거가 도면 표준, 기존 양산 이력, CAE 결과 중 어디에 있습니까?",
    "볼트 구멍이나 커버 체결면처럼 기능 여유가 있는 항목입니까?",
  ];
  ...
  if (input.toleranceType === "roughness") {
    questions.unshift("이 표면은 반복 접촉면, 슬라이딩면, 베어링 접촉면입니까?");
  }
